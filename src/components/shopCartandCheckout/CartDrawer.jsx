    ...
    <>
      <div className="aside aside_right overflow-hidden cart-drawer" id="cartDrawer">
        <div className="aside-header d-flex align-items-center">
          ...
          <button onClick={closeCart} className="btn-close-lg btn-close-aside ms-auto"></button>
        </div>
        {cartProducts.length ? (
            ...
            {cartProducts.map((elm, i) => (
              <React.Fragment key={i}>
                <div className="cart-drawer-item d-flex position-relative">
                  <div className="cart-drawer-item__info flex-grow-1">
                    ...
                    <div className="d-flex align-items-center justify-content-between mt-1">
                      <div className="qty-control position-relative">
                        ...
                        />
                        <div onClick={() => decrementQuantity(elm.id)} className="qty-control__reduce text-start">-</div>
                        <div onClick={() => incrementQuantity(elm.id)} className="qty-control__increase text-end">+</div>
                      </div>
                  ...
                  <button onClick={() => removeItem(elm.id)} className="btn-close-xs position-absolute top-0 end-0"></button>
                </div>
        ...
        <div className="cart-drawer-actions position-absolute start-0 bottom-0 w-100">
          ...
          {cartProducts.length ? (
            <>
              <Link to="/shop_cart" className="btn btn-light mt-3 d-block">View Cart</Link>
              <Link to="/shop_checkout" className="btn btn-primary mt-3 d-block">Checkout</Link>
            </>
          ) : (
            <Link to="/shop-6" className="btn btn-light mt-3 d-block">Explore shop</Link>
          )}
        </div>
      </div>
